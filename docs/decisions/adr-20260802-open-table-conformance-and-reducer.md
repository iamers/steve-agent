...
The first reducer is a GitHub Action in this repository. Its authenticated
issuer is the Action's token and its principal is the bot identity GitHub
reports. This keeps it inside the standing non-goal of no App, no hosting, and
no service, and makes the authentication rule checkable rather than abstract.
The principal is per-repository deployment configuration; another repository
...
recurring work, and the work is owned here rather than left unowned.
...
in principle. Nothing runs until the Action exists, so work claims remain
advisory in the meantime. The matrix costs one extra step on a pass that already
happens.
